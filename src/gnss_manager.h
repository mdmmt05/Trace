...
//   4. gnssGetData()    — restituisce snapshot dei dati correnti (timestamp di ricezione reale)
// ---------------------------------------------------------------------------
...
struct GnssData {
    ...
    // Timestamp monotono (microsecondi) al momento della ricezione del fix
    uint64_t fixRxMonoUs = 0;
};
...
// Aggiorna internamente lo snapshot dei dati validi e il timestamp di ricezione
void gnssUpdate();

// Restituisce true se lat/lon e data/ora sono validi e aggiornati.
bool gnssHasFix();

// Snapshot dei dati correnti (copia dell'ultimo fix valido).
